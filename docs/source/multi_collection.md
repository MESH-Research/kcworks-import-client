# Using the multi-collection importer

(kcworks-multi-collection-importer-script)=

For bulk imports that span multiple collections, use [`kcworks-multi-collection-importer`](https://github.com/MESH-Research/kcworks-import-client) (`kcworks_import_client.multi_collection_importer`). Install with the `yaml` extra if you use YAML manifests (`pip install -e ".[yaml]"`). It reads a JSON or YAML **manifest**, creates any missing collections via the communities API, optionally links parent/child collections, and then calls the same import logic as the single-collection importer for each entry.
...
- `no_updates` — `true`/`false` for that collection; overrides `--no-updates` for the entry. Default is to allow metadata updates on existing matches (with file reconciliation as in {ref}`import-existing-record-files`).
...
See [Collection hierarchy and subcollection requests](https://mesh-research.github.io/knowledge-commons-works/admin_guide/collection_hierarchy.html) for the full operator documentation on collection hierarchy.
...
| `--no-updates`           | Default block on metadata updates for existing matches; overridable per entry. Default: allow updates (see {ref}`import-existing-record-files` for files). |
